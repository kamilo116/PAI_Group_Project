...
class ReviewRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) {
  ...
  class ReviewTable(tag: Tag) extends Table[Review](tag, "review") {
    ...
    def order_product_id = column[Int]("order_product_id")
    def mark = column[Int]("mark")
    def review_content = column[String]("review_content")

    def * = (review_id, order_id, product_id, order_product_id, mark, review_content) <> ((Review.apply _).tupled, Review.unapply)
  }

  val review = TableQuery[ReviewTable]

  def create(order_id: Int, product_id: Int, order_product_id: Int, mark: Int, review_content: String): Future[Review] = db.run {
    (review.map(r => (r.order_id, r.product_id, r.order_product_id, r.mark, r.review_content))
      returning review.map(_.review_id)
      into {
        case ((order_id, product_id, order_product_id, mark, review_content), review_id) =>
          Review(review_id, order_id, product_id, order_product_id, mark, review_content)
      }
    ) += ((order_id, product_id, order_product_id, mark, review_content))
  }
