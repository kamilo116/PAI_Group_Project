...
class OptionRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) {
  ...
  def create(option_group_id: Int, option_value: String): Future[models.Option] = db.run {
  ...
  def list(): Future[Seq[models.Option]] = db.run {
    option.result
  }
  ...
  def findById(id: Int): Future[scala.Option[models.Option]] = db.run {
    option.filter(_.option_id === id).result.headOption
  }

  def findByGroupId(id: Int): Future[Seq[models.Option]] = db.run {
    option.filter(_.option_group_id === id).result
  }
  def update(newValue: Option) = db.run {
